...
if (token) {
  ...
  bot.onText(/\/start/, (msg) => {
    bot.sendMessage(msg.chat.id, "Bot activo 🔥").catch((err) => {
      console.error("[Telegram] Error al responder /start:", err.message);
    });
  });

  bot.on("message", (msg) => {
    if (msg.text && msg.text.startsWith("/start")) return;
    bot.sendMessage(msg.chat.id, "✅ Mensaje recibido").catch((err) => {
      console.error("[Telegram] Error al responder mensaje:", err.message);
    });
  });
  ...
  console.warn("[Telegram] TELEGRAM_TOKEN no configurado. El bot no recibirá comandos.");
}
...
(async () => {
  ...
  setInterval(() => {
    ciclo().catch((err) => {
      console.error("[Ciclo] Error en ciclo de análisis:", err.message);
    });
  }, 5000);
